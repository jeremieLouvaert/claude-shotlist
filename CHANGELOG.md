...
### Added
- `scripts/remix.py` — phase 1 (`--brief`) parses the report's Shot prompts into `remix.md` with `first_frame_prompt` / `last_frame_prompt` / `video_prompt` markers per shot plus a global `style_keeper`; phase 2 (`--emit`) builds ONE combined `comfy/remix_workflow.json` — section A stills (reference frame + shared prompt → Nano Banana Pro `GeminiImage2Node` AND `gpt-image-2` `OpenAIGPTImageNodeV2` per first/last), section B video (picked stills → Seedance `ByteDance2FirstLastFrameNode` AND `GeminiVideoOmni` first→image_1/last→image_2) — plus `comfy/input/` with renamed reference frames. Both engines emit per shot with the non-chosen one muted (`--stills-engine nano|gpt`, `--video-engine seedance|omni`); switching engines is a UI mute-toggle, not a re-emit. Both phases refuse to run over unfilled markers.
- `scripts/comfy_templates.json` — nine node templates, captured verbatim from a real ComfyUI install's output-PNG metadata (widgets_values ordering is undocumented surface; captured templates load, transcribed ones drift), scrubbed of creative content.
- SKILL.md Step 6 (Remix) + four failure-mode rows; 7 new tests (remix.md schema, refusal paths, graph integrity: unique ids, link endpoint/slot correctness, per-shot node counts, engine muting, style_keeper propagation). Suite: 19.
