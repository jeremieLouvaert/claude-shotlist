...
3. **Emit the workflow:**
   ...
   ```
   Writes ONE combined `comfy/remix_workflow.json` plus `comfy/input/shotNN_ref.*` (renamed reference frames). The graph has two labelled sections: **A — STILLS**, two passes per shot — pass 1 generates the FIRST frame from the reference; pass 2 generates the LAST frame conditioned on `shotNN_first.png` (the approved pick), each through Nano Banana Pro `GeminiImage2Node` AND `gpt-image-2` `OpenAIGPTImageNodeV2` sharing one prompt — and **B — VIDEO** (picked first/last stills + video prompt → Seedance `ByteDance2FirstLastFrameNode` AND `GeminiVideoOmni` with first→image_1 / last→image_2). Every generation node is wrapped in the hash-vault caching triad (`DeterministicHashVault` → `HashVaultSave` → `LazyAPISwitch`, keyed on prompt + conditioning images), so re-runs only hit the API for branches whose inputs changed. Both engines are emitted per shot; the non-chosen one is **muted** — the user switches engines by muting/unmuting in the UI, no re-emit. Relay the printed handoff: drop `comfy/input/*` into ComfyUI input → load → run A pass 1 → pick firsts, rename to `shotNN_first.png`, into input → run A pass 2 → pick lasts, rename to `shotNN_last.png`, into input → run B; everything saves under `output/remix/` with engine-suffixed prefixes.
